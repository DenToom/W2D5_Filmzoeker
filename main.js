// maken const, let op verwijderen { "Movies":  van originele data !!
...
document.addEventListener('DOMContentLoaded', function (event) {
 ...
 // extra info gezocht aanaande DOM (DOMContentLoaded) en eventlistener (event)
// https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener#The_event_listener_callback
// https://www.geeksforgeeks.org/javascript-addeventlistener-with-examples/


const list = document.getElementById("posters_img");
//Definition and Usage. The getElementById() method returns the element that has
//the ID attribute with the specified value. This method is one of the most common
// methods in the HTML DOM, and is used almost every time you want to manipulate, 
//or get info from, an element on your document.

// Posters Let op! <div class ...
let posters_img = '<div class="posters_img">';

// reduce
posters_img += movies.reduce((previous, current)=>{
    return previous + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="225px"/></a><h5>${current.Title}</h5></div>`;
}, " ");

posters_img += '</div>';
list.innerHTML = posters_img; 
     


// function filterChartData([movies]) {
//   return movies.filter((Year) => {
//     return filterKeys.every((year) => year[Year] >= 2014);
//   });
// }
// console.log('Result #1: ',filterChartData(["Title"]))

// movies filter
const last_release1 = movies.filter((year)=> year.Year >= 2014);
const avengers2 = movies.filter((film)=>film.Title.includes("Avengers"));
const x_men3 = movies.filter((film)=>film.Title.includes("X-Men"));
const princess4 = movies.filter((film)=>film.Title.includes("Princess"));
const batman5 = movies.filter((film)=>film.Title.includes("Batman"));
const all6 = movies.filter((film)=>film.Title.includes(" "));
// series filter
const series7 = movies.filter((film)=>film.Type.includes("series"));

// Movies filter per cat.: All, Latest Release, Avengers, X-Men, Princess, Batman, additonal: Series. 
let latest = '<div class="posters_img">';
latest += last_release1.reduce((prev, current)=>{
...
let avengers = '<div class="posters_img">';
avengers += avengers2.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
avengers += '</div>';

let x_men = '<div class="posters_img">';
x_men += x_men3.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
x_men += '</div>';

let princess = '<div class="posters_img">';
princess += princess4.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
princess += '</div>';

let batman = '<div class="posters_img">';
batman += batman5.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
batman += '</div>';

let all = '<div class="posters_img">';
all += all6.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
all += '</div>';
...
//Series
let series = '<div class="posters_img">';
series += series7.reduce((prev, current)=>{
  return prev + `<div class= "poster"><a href="https://www.imdb.com/title/${current.imdbID}/">
    <img src="${current.Poster}" height="175px"/></a><h1>${current.Title}</h1></div>`;
}, " ");
series += '</div>';

//Movies Buttons per cat.
// const latestButton = document.getElementById("latest");
const latestRButton = document.getElementById("last_release1");
const avengersRButton = document.getElementById("avengers");
const xmenRButton = document.getElementById("x_men");
const princessRButton = document.getElementById("princess");
const batmanRButton = document.getElementById("batman");
const allRButton = document.getElementById("all");

//Series
const series2RButton = document.getElementById("series");



// Movies
latestRButton.addEventListener('click', (event)=>{
  list.innerHTML = latest;   
});
avengersRButton.addEventListener('click', (event)=>{
  list.innerHTML = avengers;   
});
xmenRButton.addEventListener('click', (event)=>{
  list.innerHTML = x_men;   
});
princessRButton.addEventListener('click', (event)=>{
  list.innerHTML = princess;   
});
batmanRButton.addEventListener('click', (event)=>{
  list.innerHTML = batman;   
});
allRButton.addEventListener('click', (event)=>{
  list.innerHTML = all;   
});

// Series
series2RButton.addEventListener('click', (event)=>{
